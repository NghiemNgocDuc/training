...
\begin{abstract}
  ...
  2.5$\times$ degradation from the anchor consistent with domain shift.
  There flat $N_m\mu_{\mathcal T}$ catastrophically fails ($+2.14$), plain
  affine $aE+b$ fixes bulk shift ($-0.09$ [$-$0.14,$-$0.05]) but makes
  $\Lambda_m$ redundant ($p\ge0.51$), and gauge-invariant CAAT
  $\tilde E_m=aE_m+b_0+\sum b_e N_e$ doubles to $-0.177$
  [$-$0.244,$-$0.109] and rescues the worst decile by $-1.16$
  [$-$1.40,$-$0.94] via $N_e$ counts.
\end{abstract}
...
and any correction built on that decomposition should respect its gauge.
On Exp-DB flat $N_m\mu_{\mathcal T}$ catastrophically fails, a plain affine
fixes the bulk shift but makes $\Lambda_m$ redundant, and a composition-aware
affine $aE+b_0+\sum b_e N_e$ — still gauge-invariant via $N_e$ — doubles the
gain and rescues the worst decile.
...
per-atom features lie in the typical region of the training GMM. Atom-level
checks show their gated atoms sit near the pool mean. Rewriting them moves
values without information. A cutoff-radius explanation is also ruled out.
...
$\bar\lambda=0.8014$, and as ablations we keep constant $\lambda=1.0$ on the top
quartile of $\sigma_{mi}$ (2{,}904 atoms, $\sigma_{mi}\ge0.097158$),
trust-weighted replacement of gated atoms, a naive pool-mean shift, and a
random-shift placebo. This whole family is atom-level, and it is the naive
approach. The next subsection shows why it is structurally capped.
...
(RMSE 0.7696, R$^2$ 0.9646) on the 129 test molecules. This is the only
per-fold model test MAE that is fully regenerable from archived artifacts.
The fold-0 split, checkpoints, and predictions are sha256-verifiable.
The recorded five-fold headline is $0.549 \pm 0.024$ kcal/mol
(per-fold 0.515/0.537/0.581/0.570/0.540; pooled RMSE 0.921, R$^2$ 0.9427).
The folds 1--4 prediction files were lost with a destroyed training
environment. We report their recorded aggregates throughout. All correction
...
$2.49\times10^{-14}$~kcal/mol. The analytic formula matches the observed VW
shift to $2.49\times10^{-14}$, so the invariance is exact, not only
empirically small, and follows analytically from Methods.
...
As physics reference we score GBn2 (Gasteiger/mbondi3/Still GB,
$\varepsilon_{\text{in}}=1/78.5$) on the 642 conformers (deterministic,
bit-identical). Raw MAE is unusable (slope $\approx$0.11--0.16, $6$--$9\times$
overestimate), so we apply \emph{in-sample affine calibration} favoring the baseline,
\[
\hat{y}_{\mathrm{cal}} = a\, y_{\mathrm{GBn2}} + b ,
\]
with $(a, b)$ least-squares fitted on the same fold's test molecules. Even
so, fold-0 scaled MAE is 2.13 kcal/mol, and this is $4.21\times$ worse than the
fold-0 model MAE of 0.5059 ($36.1\times$ raw). The scaling is stable across
folds (Table~\ref{tab:gbn2} in Appendix~\ref{app:gbn2}), and GBn2 remains useful for rank-ordering (Kendall $\tau \approx 0.54$).
...
Two caveats. (1) retrained anchor 0.5934 vs recorded 0.549 $\pm$0.024
(+0.044 systematic, run-to-run/settings drift), we compare vs same-run anchor;
(2) external labels have no per-molecule uncertainties (literature compilations).
...
Flat GIMS with FreeSolv $N_m\mu_{\mathcal T}$ catastrophically fails
(GIMS $+2.14$, VW $+2.14$, uniform $+2.56$, all CIs harm; gauge audit still
1.96e-7 vs 14.66). Failure is domain-specific: ExpDB shifts are extensive and
element-dependent (CompSol/Abraham vs FreeSolv). Plain per-fold affine
$aE+b$ (Nelder--Mead, 2 params, same 5-fold splits) fixes bulk shift
($-0.09$ [$-$0.14,$-$0.05], 5/5 folds negative) but $\Lambda_m$ adds nothing —
same $aE+b$ on raw without $\Lambda_m$ is NS ($p\ge0.51$), so the correction
is delocalized and $\Lambda_m$ is obsolete.

Composition-Aware Affine Transfer (CAAT) restores locality gauge-invariantly:
$\tilde E_m=aE_m+b_0+\sum_e b_e N_e$, $N_e=\sum_i\mathbb{I}(e_i=e)$ (Total,
C, O, N, H, S, F, Cl, Br, P). Since $\sum_i g_{mi}=0$ leaves $N_e$ invariant,
CAAT respects the same gauge as GIMS while capturing element-specific bias
(L2 $10^{-4}$ on $b_e$, same splits). 5-fold CV: $-0.177$ [$-$0.244,$-$0.109]
all620, $-0.310$ [$-$0.494,$-$0.123] $Q_{\rm spread}$, $-1.16$
[$-$1.40,$-$0.94] WDec10 — doubling plain-affine ($p=1.00$) and rescuing the
worst decile.

\begin{table}[t]
  \caption{Exp-DB transfer ($\Delta$MAE, 3-seed, 10k bootstrap; positive$=$worse).
  Flat $N_m\mu_{\mathcal T}$ hurts; affine/CAAT fitted per fold on validation.}
  ...
  \begin{tabular}{lccc}
    ...
    affine (plain $aE+b$) & $-0.09$ [$-$0.14,$-$0.05] & $-0.12$ [$-$0.23,$-$0.01] & $-0.49$ [$-$0.66,$-$0.31] \\
    CAAT ($aE+b_0+\sum b_eN_e$) & $\mathbf{-0.18}$ [$\mathbf{-0.24}$,$\mathbf{-0.11}$] & $\mathbf{-0.31}$ [$\mathbf{-0.49}$,$\mathbf{-0.12}$] & $\mathbf{-1.16}$ [$\mathbf{-1.40}$,$\mathbf{-0.94}$] \\
    VW (flat) & $+2.14$ [$+$1.92,$+$2.36] & $+2.77$ [$+$2.22,$+$3.34] & $+1.68$ [$+$0.65,$+$2.73] \\
    uniform (flat) & $+2.56$ [$+$2.30,$+$2.82] & $+2.60$ [$+$2.09,$+$3.10] & $+1.53$ [$+$0.51,$+$2.50] \\
    \bottomrule
  \end{tabular}
\end{table}
% VERIFIED. Plain affine all620 -0.0893 [-0.135,-0.045], Q_spread -0.1186 [-0.231,-0.005], WDec10 -0.4889 [-0.659,-0.307] = node_refinement/plain_affine_control/plain_affine_results.csv; CAAT all620 -0.1769 [-0.244,-0.109], Q_spread -0.3104 [-0.494,-0.123], WDec10 -1.1634 [-1.396,-0.936], vs plain p=1.00/0.998/1.00 = node_refinement/composition_aware_affine/composition_aware_results.csv and composition_aware_report.json; identical KFold(5,shuffle,42) splits and Nelder-Mead settings; gauge-invariant via N_e counts.

Per-fold raw MAEs 1.61/1.50/1.47/1.55/1.50; ensemble 1.475 (RMSE 2.17,
$R^2$ 0.854, $\tau=0.78$) — 2.5$\times$ anchor, expected for CompSol/Abraham
literature labels vs FreeSolv. Worst errors (phosphine oxides $-0.6$ to
$-5.9$ vs $-11$ to $-17$, long esters, steroids) are element-specific
out-of-domain chemistry CAAT rescues ($-1.16$ WDec10).

\section{Discussion and limitations}

\paragraph{Any additive readout has a gauge.} The lesson goes beyond FreeSolv: any additive molecular quantity shares
the same symmetry and DimeNet, SchNet, PaiNN, NequIP, MACE all have this form,
so corrections should avoid an arbitrary gauge choice. GIMS does so by
discarding the non-identifiable $\mathrm{Cov}(\lambda_{mi},P_{mi})$ while
keeping $\Lambda_m$ heterogeneity. The same symmetry explains external
transfer: CAAT $\tilde E_m=aE_m+b_0+\sum_e b_e N_e$ is also exactly
gauge-invariant because $N_e=\sum_i\mathbb{I}(e_i=e)$ are macroscopic counts
with $\sum_i g_{mi}=0$ leaving $N_e$ unchanged, while capturing
element-dependent systematic shifts.
...
\paragraph{Implications for practice.} If an ensemble is available, GIMS needs
only per-atom variances and $N_m\mu_{\mathcal T}$ (training-free, inductive).
For domain transfer where $N_m\mu_{\mathcal T}$ fails, CAAT
$aE+b_0+\sum b_e N_e$ is the extensive, gauge-invariant correction — same
5-fold CV, no retraining, and it doubles the affine gain while rescuing
the worst decile.
...
are model-derived. Intervals cover model variation only. (iv) Folds 1--4
survive only as aggregates. Claims on them inherit that status. Same-run
anchor 0.5934 vs recorded 0.549 $\pm$ 0.024 is disclosed. (v) gradient-12
...
No family-wise correction across 25 comparisons. (vii) Atom-level target
search. six designs were tested (variance-moderated, element-conditional,
two-stage, magnitude-adaptive, hierarchical, correlation-aware). All operate
at atom level. They are now understood through the gauge lens. Each reduces
to VW or underperforms it. The one validation win (hierarchical, $-0.03$)
reversed on test ($+0.04$ to $+0.08$) and did not survive H1. This is
convergent evidence that atom-level correction is capped. No correction
beyond H2 was applied. (viii) GIMS reference mean is train-only (411/7{,}389).
...
(x) External set has literature label noise and generated conformers; 5-fold
CV is not prospective and rare elements (P, Br, S) have high variance (L2
$10^{-4}$). (xi) GBn2 comparison is in-sample affine-calibrated, favoring
the baseline, yet model wins $>4\times$. (xii) Deterministic numbers
reproduce to machine precision. Bootstrap CIs vary up to 0.30 across runs.
They are noise-banded. Point estimates are exact.
...
\paragraph{Shrinkage and pooling.} Shrinking toward a common mean is classical~\cite{efron1977}. We show it transfers to held-out molecules. GIMS is the molecular-level, gauge-invariant version for in-domain correction and CAAT is the extensive, element-aware version for cross-domain transfer — both exactly gauge-invariant (GIMS via $\Lambda_m$, CAAT via $N_e$).
...
repairs it. Atom-level weighting fails because the additive decomposition is
not unique and a zero-sum reallocation leaves $E_m$ unchanged while moving
$\hat E_{VW}$ — gauge freedom — and GIMS aggregates to one $\Lambda_m$ per
molecule, making it exactly invariant while beating uniform significantly on
four of five pops on both test and H2 and beating VW point-wise on five of
five (NS). External transfer on 620 ExpDB degrades to MAE 1.475 ($R^2$ 0.854,
2.5$\times$) but remains rank-correlated; flat $N_m\mu_{\mathcal T}$
catastrophically fails ($+2.14$), plain $aE+b$ fixes the bulk ($-0.09$,
$p\ge0.51$ vs $\Lambda$-weighted) but obsoletes $\Lambda_m$, and
composition-aware $aE+b_0+\sum b_e N_e$ — also exactly gauge-invariant via
$N_e=\sum_i\mathbb{I}(e_i=e)$ — doubles to $-0.177$ and rescues the worst
decile by $-1.16$ (all $p=1.00$ vs plain). The broader lesson is that any
correction built on an additive readout should respect its gauge.
...
{\it Journal of Computer-Aided Molecular Design} {\bf 28}(7):711--720, 2014.
...
{\it Scientific Data} {\bf 11}:742, 2024.
...
{\it Journal of Chemical Information and Modeling} {\bf 62}(8):1840--1848, 2022.
...
{\it Journal of Chemical Theory and Computation} {\bf 3}(1):156--169, 2007.
doi:10.1021/ct600085e.

\bibitem{klamt1995}
A. Klamt,
Conductor-like Screening Model for Real Solvents: A New Approach to the Quantitative Calculation of Solvation Phenomena,
{\it Journal of Physical Chemistry} {\bf 99}(7):2224--2235, 1995.
...
{\it Chemical Engineering Journal} {\bf 418}:129307, 2021.
...
{\it Journal of Cheminformatics} {\bf 13}:56, 2021.
...
{\it Scientific American} {\bf 236}(5):119--127, 1977.
...
Two follow-up GIMS variants test the limits of invariance, and both are reported as
