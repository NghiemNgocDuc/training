...
water — is central to drug discovery~\cite{mobley2014freesolv}. Graph Neural Networks now predict it to within an error of a few tenths of a kcal/mol on
FreeSolv~\cite{zhang2022}, but average accuracy is not enough: a model
...
\begin{equation}\label{eq:add_Gauge}
    P_{mi}^{(k)\prime} = P_{mi}^{(k)} + g_{mi}, \qquad \sum_{i}g_{mi}=0
\end{equation}
...
-N_m\operatorname{Cov}_{i\in m}\left(\lambda_{mi},P_{mi}^{(k)}\right)
\end{equation}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\columnwidth]{image1.pdf}
  \caption{The same five-seed ensemble produces per-atom reliability
$\lambda_{mi}$. Variance-weighted (VW) shrinkage shrinks each atom
individually, then sums the results — a Gauge-dependent operation, shifting
by 3.26~kcal/mol under the null perturbation of Section~\ref{sec:method}.
Gauge-Invariant Molecular Shrinkage (GIMS) instead averages $\lambda_{mi}$
to a single molecular strength $\Lambda_m$ and shrinks once — a
Gauge-invariant operation, shifting by only $\sim$0 kcal/mol under the same
perturbation.}
...
\textbf{Model:} We use a DimeNet++-style architecture, specifically
...
\textbf{Training:} Training proceeds in three stages. Stage 1 pretrains the
...
\textbf{Uncertainty:} To estimate node-level uncertainty, we train a
...
\textbf{Evaluation Protocol:} We use a frozen five-fold split: molecules are
...
\textbf{Metrics and Test Populations:} We report MAE, RMSE, $R^2$, and Kendall $\tau$, along
...
per-fold checkpoints. All correction arms below use the three-seed subset
...
Table~\ref{tab:arms} shows every shrinkage arm improves on the raw
ensemble mean. Variance-weighted (VW) shrinkage is the best atom-level
arm, with a 95\% CI excluding zero on four of five populations (full
intervals in Appendix~\ref{app:protocol}); its gain on gradient-12 is not
significant. The strength-matched uniform baseline ($\bar\lambda =
0.8014$) recovers 87--89\% of VW's gain, and the difference between VW and
uniform is small and not significant on any population. In other words,
weighting the correction by per-atom variance adds little beyond what a
single global shrinkage constant already achieves. Trust-weighted
shrinkage is the exception, actively harming gradient-12, which the
covariance identity from Section~\ref{sec:method} explains as the ceiling
on atom-level sophistication. Naive and random controls show no effect, a
randomized-neighbor control reproduces the trust result, and the pattern
holds on the independent H2 holdout.
...
\textbf{Permutation Control:} We randomly shuffle the 129 $\Lambda_m$ values
...
\textbf{Holdout:} Recalibrating on H1 ($n=64$) and scoring on H2 ($n=65$)
...
\textbf{Reference Reconciliation:}\label{para:ref-reconciliation} We also tried computing the reference
...
numbers on all129 for every arm. We still report the train-only version as
our default, since it does not use any test-time information and is
therefore safe to deploy.

\textbf{Bias--Variance:} The covariance identity from Section~\ref{sec:method}
...
the flat shrinkage prior does not: every correction arm makes predictions
...
\begin{table}[htbp]
  \caption{External transfer of the correction arms, change in test MAE
...
\textbf{Calibration Grid:} We calibrate $\tau^2$ on the validation split,
...
\textbf{Confidence Intervals for Tables~\ref{tab:arms} and~\ref{tab:gims}:}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{fig_arms_forest.pdf}
...
than the plain variance-weighted (VW) arm, which uses a fixed target (the
...
\textbf{GIMS/VW blend:} We blend the two corrections,
...
\textbf{CAAT:} Composition-Aware Affine Transfer extends the external-set
...
precision for every arm and molecule). This lets us explain the small gap
...
On the four main populations, about two-thirds of every arm's MSE reduction
comes from bias-squared (0.65--0.67 across arms), so the arms mainly differ
in the remaining third, the variance term. On all129, VW and the gated arm
...
gated arms increase MSE outright there. Table~\ref{tab:bias-var} gives the
full per-arm, per-population breakdown. Note this decomposition is in MSE
