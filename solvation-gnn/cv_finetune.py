...
# Ensure we run from root so relative paths work
os.chdir(_root)
...
def main():
    ...
    parser.add_argument("--output_dir", default="cv_results")
    ...
    # Sort by expt for balanced folds
    ...
    if args.n_conformers > 1:
        ...
        for fold in folds_to_run:
            ...
            ens_preds_check = np.array([np.mean(conf_preds[mid]) for mid in tids])
            ens_expts_check = np.array([all_labels[mid]["expt"] for mid in tids])
            cm, cr, cr2 = evaluate(ens_preds_check, ens_expts_check)
    ...
    try:
        ...
        ax.set_title(f"Fine-tuned Ensemble  |  MAE={em:.3f}  RMSE={er:.3f}  R²={er2:.4f}")
        ax.legend()
        ax.set_aspect("equal")
        fig.tight_layout()
