"""AIMNet2 transfer arms: uniform (val-calibrated) + GIMS/VW/full check.

- Uniform lambda_bar: grid-searched on AIMNet2-FreeSolv val (NOT matched to the
  degenerate zero Lambda), frozen, then applied to FreeSolv-test pops,
  FlexiSol-290 and Guthrie-442. mu_T per-seed from AIMNet2 train pool.
- GIMS/VW use AIMNet2 val-selected tau* -> identical to raw by construction;
  Lambda distributions reported to show why. 10k paired bootstraps, tqdm.
Outputs -> aimnet_freesolv/fold0_ensemble/transfer/aimnet_transfer_arms.csv/json
Usage: py -V:3.12 aimnet_freesolv/aimnet_transfer_arms.py
"""
...
TR = os.path.join(AIM_DIR, "transfer")
...
def main():
    ...
    found = sorted(int(os.path.basename(p).split("seed")[1].split(".pkl")[0])
                   for p in glob.glob(os.path.join(
                       AIM_DIR, "peratom_aimnet_seed*.pkl")))
    if found:
        SEEDS = found
    print(f"[seeds] AIMNet2 ensemble: {SEEDS} (K={len(SEEDS)})", flush=True)
    t0 = time.time()
    # ---- AIMNet2 FreeSolv pools
    pred = pd.read_csv(os.path.join(AIM_DIR, "aimnet_seed_predictions_all642.csv"))
    ...
    Ntr = {}
    for s in SEEDS:
        d = pickle.load(open(os.path.join(AIM_DIR, f"peratom_aimnet_seed{s}.pkl"),
                             "rb"))
    ...
    # ---- Lambda audit (AIMNet2 per-atom spread, DimeNet tau* scale)
    for name, path in (("freesolv-train", None),):
        pass
    lam_stats = {}
    D = {s: pickle.load(open(os.path.join(
        AIM_DIR, f"peratom_aimnet_seed{s}.pkl"), "rb")) for s in SEEDS}
    ...
    print(f"[audit] AIMNet2 train-atom std: mean={np.sqrt(s2).mean():.5f} "
    ...
    # transfer sets
    for setn, fn, idc in (("flexisol", "aimnet_flexisol_permol.csv", "id"),
                          ("guthrie", "aimnet_guthrie_permol.csv", "id")):
    ...
    res.to_csv(os.path.join(TR, "aimnet_transfer_arms.csv"), index=False)
    print(res.round(3).to_string(index=False), flush=True)
    json.dump({"lambda_star": best, "mu": mu,
               "rows": res.to_dict("records"),
               "runtime_s": round(time.time() - t0, 1)},
              open(os.path.join(TR, "aimnet_transfer_arms.json"), "w"), indent=2)
