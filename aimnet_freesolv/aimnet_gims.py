"""AIMNet2 GIMS / VW / uniform on FreeSolv fold-0 — mirrors the DimeNet GIMS audit.

Inputs: aimnet_freesolv/fold0_ensemble per-atom + predictions (this repo),
...
Method: identical to aqm-spice2/.../gauge_invariant_shrinkage.py, except
  arrays come from AIMNet2 atomic_shift+Coulomb-split (gate exact to ~3e-10).
  tau recalibrated on the AIMNet2 102-mol val split over its own 37-pt grid.
Outputs -> aimnet_freesolv/fold0_ensemble/analysis/.
Usage: py -V:3.12 aimnet_freesolv/aimnet_gims_analysis.py
"""
...
OUT = os.path.join(AIM_DIR, "analysis")
...
def main():
    ...
    if found:
        SEEDS = found
    K = len(SEEDS)
    print(f"[seeds] AIMNet2 ensemble: {SEEDS} (K={K})", flush=True)
    t0 = time.time()
    nodes = pd.read_csv(os.path.join(AIM_DIR, "aimnet_node_contributions.csv"))
    pred = pd.read_csv(os.path.join(AIM_DIR,
                                    "aimnet_seed_predictions_all642.csv"))
    ...
    res.to_csv(os.path.join(OUT, "aimnet_gims_results.csv"), index=False)
    ...
    json.dump(rep, open(os.path.join(OUT, "aimnet_gims_report.json"), "w"),
              indent=2)
