...
parser = argparse.ArgumentParser(
    description="Stage 2: Train Correction DimeNetPlus on AQM-sol with frozen vacuum model"
)
...
parser.add_argument("--gas_hdf5", type=str, default=None,
                    help="Path to AQM-gas.hdf5 for optional pairing")
...
def build_dimenet(is_vacuum=False):
    num_blocks = args.num_blocks + 1 if is_vacuum else args.num_blocks
    return DimeNetPlus(
        in_channels=1,
...
vacuum_model = build_dimenet(is_vacuum=True)
...
correction_model = build_dimenet(is_vacuum=False)
