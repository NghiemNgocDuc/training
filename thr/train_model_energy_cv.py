#### This is the same code as train_model_DimeNetwithFeatures_energy, but use a fixed epoch for each fold instead of early stopping.
...
def test_model_individual(test_loader, model):
    ...
    with torch.enable_grad():
        for data in test_loader:
            ...
            df_all = pd.concat([df, df_gt], axis=1)
            save_path = f"results_nogbforce/{sample_name}_prediction.csv"
...
for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)):
    ...
    if args.load:
        model_save_path = f"results_nogbforce/fold_{fold + 1}_model.pt"
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.4f}, Time: {epoch_duration:.2f} sec")

        if not os.path.exists("results"):
            os.mkdir("results")
        # Save the model for this fold
        model_save_path = f"results/fold_{fold + 1}_model.pt"
