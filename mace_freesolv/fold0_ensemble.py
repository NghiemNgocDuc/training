"""MACE fold-0 ensemble (GIMS-ready) — train + per-atom dump.
...
What it does (every step has a tqdm progress bar):
  ...
  2. For each seed in --seeds (default 42,123,999):
     a. Train MACE Stage-B from the SAME Stage-A checkpoint (only seed changes).
...
Usage (Vast GPU, see run_mace_ensemble.sh):
  python mace_freesolv/fold0_ensemble.py --seeds 42,123,999 --device cuda
  python mace_freesolv/fold0_ensemble.py --seeds 42 --quick_test --device cuda
  python mace_freesolv/fold0_ensemble.py --skip_train --seeds 42,123,999 --device cuda
...
def train_one_seed(seed, tr, va, te, device, out_root, cfg, init_ckpt, quick):
    ...
    set_seed(seed)
    print(f"\n{'='*64}\n  SEED {seed}  (fold-0 only, init={os.path.basename(init_ckpt)})"
          f"\n{'='*64}", flush=True)
    ...
    with open(os.path.join(seed_dir, "metrics.json"), "w") as f:
        json.dump({"seed": seed, "n_train": len(tr), "n_val": len(va),
                   ...
                   "init_checkpoint": init_ckpt}, f, indent=2)
...
def main():
    ...
    ap.add_argument("--init_checkpoint", default=DEFAULT_STAGE_A)
    ap.add_argument("--hdf5", default=DEFAULT_HDF5)
    ap.add_argument("--output_dir",
                    default=os.path.join(REPO_ROOT, "mace_freesolv", "fold0_ensemble"))
    ...
    print(f"[run] hdf5={a.hdf5}\n[run] init={a.init_checkpoint}\n"
          f"[run] out={a.output_dir}", flush=True)
    assert os.path.exists(a.hdf5), f"missing HDF5 {a.hdf5}"
    if not a.skip_train:
        assert os.path.exists(a.init_checkpoint), \
            f"missing Stage-A checkpoint {a.init_checkpoint}"
    ...
    if not a.skip_train:
        ...
        for s in tqdm(seeds, desc="OVERALL seeds", unit="seed"):
            r = train_one_seed(s, tr, va, te, device, a.output_dir, cfg,
                               a.init_checkpoint, a.quick_test)
            summary[str(s)] = r
