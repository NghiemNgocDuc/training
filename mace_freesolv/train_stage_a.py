...
def main():
    parser = argparse.ArgumentParser(
        description="Stage A: fine-tune MACE-OFF23 on AQM solvation (dG = E_sol - E_gas)")
    ...
    parser.add_argument("--output_dir", type=str, default=None, help="Output dir (default: mace_freesolv/results_stage_a)")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--freeze_atomic_energies", action="store_true", default=True)
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results_stage_a")
    ...
    meta = {
        ...
        "seed": args.seed,
        "note": "dG = E_sol - E_gas (ePBE0+MBD), eV converted via 23.0605 kcal/mol",
    }
    with open(os.path.join(args.output_dir, "stage_a_meta.json"), "w") as f:
        json.dump(meta, f, indent=2)

    print(f"\n{'='*60}")
    print(f"  STAGE A DONE: checkpoint at {checkpoint_path}")
