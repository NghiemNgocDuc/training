...
parser.add_argument('-ra','--radius',default=5.0,type=float,help='radius')
...
class CustomDataset:
    def __init__(self, root, istrain=False):
        self.dataset = self.get_data(root, istrain)
        pass

    def get_data(self, folder_path, istrain=False):
        Dataset = []
        feats = []
        feat_names = []
        ...
        for sample_dir in sample_dirs:
            ...
            if istrain:
                ...
                data_sample = Data(x=torch.tensor(X, dtype=torch.float),
                                   pos=torch.tensor(coordinates, dtype=torch.float, requires_grad=True))
...
def test_model_individual(test_loader, model, gt=True):
    """
...
data_train = CustomDataset(root="../Data/THR_data_19SB_OPC_GBn2", istrain=True)
data_test = CustomDataset(root="../Data/"+output_folder, istrain=False)
...
val_subset = Subset(data_train, val_idx)
...
if not args.testonly:
    ...
    model_save_path = f"train_full_model_cutoff"+str(args.radius)+".pt"
    ...
    test_model_individual(test_loader, model, gt=False)
