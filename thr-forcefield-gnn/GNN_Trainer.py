...
class Trainer:
    def __init__(self, name='name', path='.', verbose=False, enable_tmp_dir=True, force_mode=False, device=None,random_state=161311, lr=0.01):
        self._name = name
        self._model = None
    ...
    def train_model2(self, runs, batch_size=100):
        ...
        self.initialize_optimizer(self._lr)
        self._model.train()
    ...
    def prepare_training_data_for_multiple_peptides_from_file(self,folder_path,train_test_split=0.8,work_dir='.'):
        ...
        # print(attr)
        atom_features = attr.loc[:, ['LJ_Radius', 'LJ_Depth', 'Mass', 'Charge', 'GB_Radius', 'GB_Screen']]
        type_one_hot = pd.get_dummies(attr["TYPE"])
        atom_features = pd.concat([atom_features, type_one_hot], axis = 1)
        ...
        for sample_dir in sample_dirs:
            ...
            filecount += 1
            forces = readXYZ(os.path.join(folder_path, "WINDOWS", sample_dir,"GBn2.frc.xyz"))
            GBn2_forces = readXYZ(os.path.join(folder_path, "WINDOWS", sample_dir,"OPC.frc.xyz"))


            data = [Data(pos=torch.tensor(coordinates[i], dtype=torch.float, requires_grad=True),
                        forces=torch.tensor([forces[i][0, 0]], dtype=torch.float).unsqueeze(1),
                        GBn2_forces=torch.tensor(GBn2_forces[i][:, 1:], dtype=torch.float),
                        atom_features=torch.tensor(atom_features, dtype=torch.float)) for i in
                    range(len(coordinates))]
            print(filecount,len(data))
            Dataset += data

        print('Constructed Dataset with %i frames ' % (len(Dataset)))
